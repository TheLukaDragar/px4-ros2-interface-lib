...
static_assert(static_cast<int>(Result::ModeFailureOther) ==
...
constexpr inline const char* resultToString(Result result) noexcept
{
  switch (result) {
    ...
    case Result::Deactivated:
      return "Deactivated";

    case Result::ModeFailureOther:
...
class ModeBase : public Context {
 public:
  ...
  struct Settings {
    ...
    Settings(std::string mode_name) : name(std::move(mode_name)) {}

    const std::string name;  ///< Name of the mode with length < 25 characters
    bool activate_even_while_disarmed{
        false};  ///< If true, the mode is also activated while disarmed if selected
  ...
  /**
   * Register the mode. Call this once on startup, unless there's an associated executor. This is a
   * blocking method.
  ...
  /**
   * Mode completed signal. Call this when the mode is finished. A mode might never call this, but
   * modes like RTL, Land or Takeoff are expected to signal their completion.
   * @param result
   */
   ...
   * setpoint types, and are used to prevent arming or trigger failsafes.
   */
